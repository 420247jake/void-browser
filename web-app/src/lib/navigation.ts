...
// Can pass just position array, or nodeId + position
export function triggerTeleport(positionOrNodeId: [number, number, number] | number, position?: [number, number, number]) {
  if (Array.isArray(positionOrNodeId)) {
    // Just position passed
    teleportTarget = {
      position: new THREE.Vector3(positionOrNodeId[0], positionOrNodeId[1], positionOrNodeId[2]),
    };
  } else {
    // nodeId + position passed
    teleportTarget = {
      position: new THREE.Vector3(position![0], position![1], position![2] + 15),
      nodeId: positionOrNodeId,
    };
  }
  isTeleporting = true;
}
